...
1. 設定画面を開き
    ...
    - [ファイルの日付フォーマット](#ファイルの日付フォーマット)にデイリーノートの日付フォーマットを設定
        ...
        - `2026-05-26(金).md` のようなファイル名なら `YYYY-MM-DD(ddd)`
            - 日本語の曜日表記を使う場合は、[ファイルの日付フォーマットのロケール](#ファイルの日付フォーマットのロケール)に`日本語 (ja)`を設定
...
// 年ごとの基本ルーチン (実施後に削除すれば予定管理としても利用可能)
毎年7月1日にやるタスク,0701
...
3. Vaultのrootに`holidays.md`を作成して以下を貼り付け
...
4. 本日のDaily Noteを開き、[Silhouette: Insert tasks]コマンドを実行
...
> [!TIP]
> チェックマークも更新したい場合は、設定: [チェックボックスのマークも更新する] を有効にします。
...
> [!IMPORTANT]
> このコマンドを利用するには、設定: [チェックボックスのマークも更新する] を有効にしてください。
...
ファイル名から日付を解析するときに使用するロケールを指定します。

曜日表記を含むフォーマットを使う場合に影響します。たとえば `YYYY-MM-DD(ddd)` で `2025-12-13(土)` のようなファイル名を使う場合は、`日本語 (ja)` を指定してください。

この設定はファイル名の日付解析に使うためのものです。[繰り返しパターン]の記法には影響しません。

| 設定値         | 対応する表記例   |
| -------------- | ---------------- |
| `English (en)` | `2025-12-13(Sat)` |
| `日本語 (ja)` | `2025-12-13(土)`  |
...
指定した計測時間に達したときに通知が表示されます。HH:MM形式で指定し、改行区切りで複数指定できます。

`例`: 以下のように設定すると『30分』『1時間』『2時間』経過したときにそれぞれ通知されます。
...
> [!WARNING]
> PC版のみ対応しています。Mobile版は未対応です。

> [!WARNING]
> 通知のタイミングは最大で1分のラグがあります。アラートの判定を1分ごとに行っているためです。
...
- 計測を開始したとき
- 計測を終了したとき
- 計測中、前回の更新から1分経ったとき (計測中は1分に1回更新)

> [!WARNING]
> 端末間で同期したときも最大で1分のラグがあります
...
| ---------------- | ------------------------------------------------------------- |
| 計測中のマーク   | [計測中]になったとき                                          |
| 計測停止のマーク | [計測中]から `Push timer` で[計測済]に変更したとき            |
| 完了のマーク     | [計測中]から `Cycle bullet/checkbox` で[計測済]に変更したとき |


> [!TIP]
> 設定値を半角スペース ` ` にすると、マークを外せます。
...
[チェックボックスのマークも更新する]: #チェックボックスのマークも更新する
